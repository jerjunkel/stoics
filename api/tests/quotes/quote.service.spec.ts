...
afterEach(() => {
  jest.clearAllMocks();
  mockRepoFindSpy.mockClear();
});

describe("Quote Service", () => {
  describe("get a quote by ID", () => {
    it("should return a single quote if ID is found", async () => {
      mockRepoFindSpy.mockResolvedValue(mockQuote);
      const quote = await sut.getAQuoteByID(mockQuote.id.toString());
  ...
  describe("get all quotes", () => {
    it("should be call repository once", async () => {
      ...
      expect(mockRepoFindAllSpy.mock.calls).toHaveLength(1);
    });
  ...
  describe("daily quote", () => {
    ...
    it("should return true if QOD is set", async () => {
      mockRepoFindAllSpy.mockResolvedValue([
        {
          ...mockQuote,
          day: mockCurrentDayNumber(),
      ...
      expect(isTodayQuoteSet).toBe(true);
    });

    it("should return false if QOD is not set", async () => {
      mockRepoFindAllSpy.mockResolvedValue([]);
      const isTodayQuoteSet = await sut.isTodayQuoteSet();
